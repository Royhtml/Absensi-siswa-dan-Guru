...
<script type="text/javascript">
   ...
   const codeReader = new ZXing.BrowserMultiFormatReader(hints, 100);
      ...
      codeReader.listVideoInputDevices()
         .then(videoInputDevices => {
            ...
            navigator.mediaDevices.getUserMedia(constraints)
               .then(stream => {
                  codeReader.decodeFromStream(stream, 'previewKamera', (result, err) => {
                     if (result) {
                        if (isCooldown) {
                           return;
                        }
                        ...
                        cekData(result.text);
                        $('#previewParent').addClass('cooldown');
                        $('#searching').text('Memproses...').removeClass('d-none');
                        setTimeout(() => {
                           isCooldown = false;
                           $('#previewParent').removeClass('cooldown');
                           $('#searching').addClass('d-none').text('Mencari...');
                        }, 2500);
                     }

                     if (err && !(err.name === 'NotFoundException' || (typeof ZXing !== 'undefined' && err instanceof ZXing.NotFoundException))) {
               ...
               .catch(err => {
                  console.warn('Resolusi HD tidak tersedia, fallback ke default:', err.message);
